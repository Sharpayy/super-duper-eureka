...
Map::Map(float map_w, float map_h, double persistence, double frequency, double amplitude, uint8_t octaves, uint32_t randomseed, Program pp, bool CPU)
{
	this->map_w = map_w;
	this->map_h = map_h;
...
uint8_t Map::getBiomeTypeAdv(float x, float y, float scale)
{
...
uint8_t Map::getBiomeThr(float x, float y, float scale, int s)
{
	float true_x = (x * scale) + (map_w / 2.0f);
	float true_y = (y * scale) + (map_h / 2.0f);
	int idx = ((int)true_y * map_w) + ((int)true_x);
...
void Map::generateMapData(double persistence, double frequency, double amplitude, uint8_t octaves, uint32_t randomseed)
{
